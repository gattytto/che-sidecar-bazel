...
RUN mkdir /projects/googleapis && git clone https://github.com/googleapis/googleapis.git /projects/googleapis && cd /projects/googleapis && ls -alh && \
    ...
    bazel run -- //:build_gen --src=google/api && bazel run -- //:build_gen --src=google/spanner && \
    bazel run -- //:build_gen --src=google/monitoring/v3 && \
    ...
    sed -i "s/\/\/google/@com_google_googleapis\/\/google/g" google/api/BUILD.bazel && \
